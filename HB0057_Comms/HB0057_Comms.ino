...
/* Create struct to keep track of peer */
typedef struct peer_t {
  uint8_t   ping_sent;
  uint8_t   mac_found;
  uint8_t   response_received;
  uint8_t  statistics_recorded;
} peer_t;

/* Instantiate an instance of struct
 *  This is just a quick dirty test, idealy
 *  this would be an array of peers, contain the mac,
 *  used in the radio setup, and status tracking.
 */
peer_t g_peer_a;
...
volatile unsigned long g_timer_screen_ms = 0;
volatile unsigned long g_timer_screen_refresh_ms = 0;
volatile unsigned long g_timer_ping_timeout_ms = 0;
...
void loop() 
{
  ...
  /* If current packet has not been recorded in statistics*/
  if(g_peer_a.statistics_recorded == STATE_RESET)
  {
    /* If all stages passed, record success */
    if((g_peer_a.ping_sent == STATE_SUCCESS)&&
       (g_peer_a.mac_found == STATE_SUCCESS)&&
       (g_peer_a.response_received == STATE_SUCCESS))
       {
          g_sucess_counter++;
          /*Do some casting for the float calculation of the ratio */
          g_sucess_ratio = (uint8_t)(((float)g_sucess_counter / ((float)g_sucess_counter+(float)g_fail_coutner))*(float)100);
          g_peer_a.statistics_recorded = STATE_SUCCESS;
       }
       
    /* If any of the stages filed, record failure */
